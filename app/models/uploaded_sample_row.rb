  ...
  def create_sample
    ...
    if existing_sample
      ...
      if errors.none?
        ...
        existing_sample.projects << project unless existing_sample.projects.exists?(project.id)
      end
    else
      proposed_sample.projects << project unless proposed_sample.projects.exists?(project.id)
  ...
  def create_sequencing_product
    ...
    if existing_product
      %i[library_prep instrument flow_cell_id strand kit targeted_capture paired_end batch].each do |field|
        ...
        @sequencing_product = existing_product if errors.none?
      end
    else
